...
namespace WoWonder.Activities.SettingsPreferences.TellFriend
{
    ...
    public class TellFriend_Activity : AppCompatActivity
    {
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                var view = MyContextWrapper.GetContentView(this, Settings.Lang,
                    Resource.Layout.Settings_TellFriend_Layout);
                if (view != null)
                    SetContentView(view);
                else
                    SetContentView(Resource.Layout.Settings_TellFriend_Layout);

                var ToolBar = FindViewById<Toolbar>(Resource.Id.settingstoolbar);
                if (ToolBar != null)
                {
                    ToolBar.Title = GetText(Resource.String.Lbl_Tell_Friends);
                ...
                var ff = FragmentManager.BeginTransaction()
                    .Replace(Resource.Id.content_frame, new TellFriend_PrefsFragment(this)).Commit();
            }
            catch (Exception e)
            {
